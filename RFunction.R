...
rFunction <- function(data, colour_name = NULL, road_files = NULL) {
  ...
  # roads <- st_read("data/raw/Roads_250k_Canvec_WGS84.shp/Roads_250k_Canvec_WGS84.shp")
  roads <- st_read(paste0(getAppFilePath("road_files"), "roads.shp"))

  data_lsl <- lapply(move::split(data), function(x) {
    Lines(list(Line(coordinates(x))), ID = namesIndiv(x))
  })
  
  data_sldf <- SpatialLinesDataFrame(
    SpatialLines(data_lsl, proj4string = data@proj4string),
    data = data.frame(idData(data), "trackId" = unique(trackId(data))),
    match.ID = F)
  ...
  if (dimcrss[1] == 0) {
    ...
    if (is.null(colour_name) | length(colour_name) == 0 | colour_name %in% names(roads_crop) == FALSE) {
      ...
      map <- ggplot() +
        ...
        scale_alpha_manual(
          name = NULL, # here a "title" could be added
